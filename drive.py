...
class DriveAPI:
    ...
    def folder_id_lookup(self, folder):
        try:
            ...
            return self.search(name=folder, files=False)[0][0]['id']
    ...
    def search(self, name:str='', pageSize:int=1, parent:str='', files=True, folders=True, pageToken:str='', recursive=False):
        ...
        if files and folders:
            ...
            mimeScript = "and mimeType!='application/vnd.google-apps.folder'"
        else:
            mimeScript = "and mimeType='application/vnd.google-apps.folder'"
        ...
        try:
            results = (
                self.service.files()
                .list(pageSize=pageSize, pageToken=pageToken, q=f"trashed = false{mimeScript}{nameScript}{parentScript}", orderBy="folder, name", fields="nextPageToken, files(id, name)")
                .execute()
            )
...
if __name__ == "__main__":
    API = DriveAPI("RPI")
    print(API.search(pageSize=1, parent="RPI"))